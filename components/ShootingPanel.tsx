...
import { Sparkles, RefreshCw, Camera, Lightbulb, Scissors, Package, CheckCircle2, Circle } from 'lucide-react';
...
export default function ShootingPanel({ project, onChange }: Props) {
  ...
  async function handleGen() {
    ...
    setLoading(true);
    try {
      const g = await genShootingGuide(project.script, project.platform);
      onChange({ shootingGuide: g, checkedGear: [], checkedShots: [] });
    } catch {
      onChange({ shootingGuide: mockShootingGuide(), checkedGear: [], checkedShots: [] });
    } finally {
      setLoading(false);
    }
  }
  ...
  function toggleGear(item: string) {
    ...
    const next = curr.includes(item) ? curr.filter(g => g !== item) : [...curr, item];
    onChange({ checkedGear: next });
  }

  function toggleShot(order: number) {
    const curr = project.checkedShots ?? [];
    const next = curr.includes(order) ? curr.filter(n => n !== order) : [...curr, order];
    onChange({ checkedShots: next });
  }

  function proceed() {
    onChange({ status: 'editing', step: 'publish' });
  }

  const guide = project.shootingGuide;
  const gearDone = (project.checkedGear ?? []).length;
  const shotsDone = (project.checkedShots ?? []).length;
  const totalGear = guide?.gear.length ?? 0;
  const totalShots = guide?.shots.length ?? 0;
      ...
      <div className="flex items-center gap-3">
        ...
        {guide && (
          <div className="ml-auto flex gap-4 text-xs text-slate-400">
            <span className={gearDone === totalGear ? 'text-emerald-400' : ''}>
              器材 {gearDone}/{totalGear}
            </span>
            <span className={shotsDone === totalShots ? 'text-emerald-400' : ''}>
              分镜 {shotsDone}/{totalShots}
            </span>
          </div>
        )}
      </div>
      ...
      {guide ? (
        <div className="space-y-5">
          ...
          <section>
            <h3 className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">
              ...
              {gearDone === totalGear && <span className="text-emerald-400">✓ 全部就绪</span>}
            </h3>
                  ...
                  <label key={item} className="flex items-start gap-3 cursor-pointer group">
                    <button
                      ...
                      className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
                        checked ? 'bg-emerald-500 border-emerald-500' : 'border-slate-600 group-hover:border-slate-400'
                      }`}
                    >
          ...
          <section>
            <h3 className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">
              ...
              {shotsDone === totalShots && <span className="text-emerald-400">✓ 全部拍完</span>}
            </h3>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={`flex-shrink-0 w-7 h-7 rounded-full border-2 flex items-center justify-center font-bold text-xs transition-all ${
                        done ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-slate-600 text-slate-400 group-hover:border-slate-400'
                      }`}>
                        {done ? '✓' : shot.order}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-sm transition-colors ${done ? 'text-slate-500' : 'text-slate-300'}`}>
                          {shot.notes}
                        </p>
                      </div>
                    </div>
        ...
        <div className="rounded-xl border border-dashed border-slate-700 p-10 text-center">
          ...
          <p className="text-slate-500 text-sm">
            {project.script ? '点击 AI 生成拍摄指南' : '请先完成脚本步骤'}
          </p>
        </div>
      )}
    </div>
